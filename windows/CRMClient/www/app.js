...
define(function(require) {
	...
	function App(){
        ...
        _self.lang = require("languageConfig");
	...
	App.prototype.setLanguage = function(filter) {
        $(filter).each(function (index, dom) {
            var langID = $(dom).attr("langID");
            if (langID) {
                var langIDSlipt = langID.split(".");
                var currLanguage = _self.lang;
                for (var i = 0; i < langIDSlipt.length; i++) {
                    currLanguage = currLanguage[langIDSlipt[i]];
                }
                $(dom).html(currLanguage[_self.langType]);
            }
        });
    };
	App.prototype.setLanguageForPanel = function(id) {
        _self.setLanguage(app.utils.panelID(id)+" *");
        _self.setLanguage("#afui #header *");
    };
